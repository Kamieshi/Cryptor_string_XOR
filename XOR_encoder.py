...
def _convert_str_to_bin(str_obj):
    _bin = ''
    for char in str_obj:
        _ascii_convert = (ord(char))
        _bin_chr = _fix_len_bine(_ascii_convert)
        _bin += _bin_chr
    return _bin


# Функция обратная _convert_str_to_bin
def _decoder_bin_to_str(bin_str):
    all_char = []
    str_out = ""
    block = ''
    for item in bin_str:
...
str_in = 'String Строка для зашифрования !"№;%:?*()!@#$%^&*('
key = 'Ключ для зашифрования'

cripto_msg = _cripto(str_in, key)
print(cripto_msg)
encript_msg = _encript(cripto_msg, key)
print(encript_msg)
